...
public class SpinnerBehavior2D implements ParticleBehavior2D {
	...
	public SpinnerBehavior2D(Vec2D attractor, float radius, float strength) {
		this(attractor, radius, strength, 0);
	}

	public SpinnerBehavior2D(Vec2D attractor, float radius, float strength, float jitter) {
	...
	public void apply(VerletParticle2D p) {
		...
		if (dist < radiusSquared) {
			Vec2D temp = new Vec2D(delta.y, -delta.x);
			temp.scaleSelf(attrStrength).scaleSelf(100f / dist);
			p.addForce(temp);
		}
	}
